...
const { apiOptions, compareDBObject, noUserApiOptions } = require('../../lib/helpers');
...
test('anthropometry creation should throw 401 if user is not provided', async () => {
  const anthropometryAPI = new AnthropometryAPI(noUserApiOptions(schema));
  const anthropometryProspect = TEST_TABLE[0](anthropometrist._id);
  anthropometryProspect.anthropometrist = anthropometrist;
  let errThrown;
  try {
    await anthropometryAPI.create(anthropometryProspect);
  } catch (e) {
    errThrown = e;
  }
...
test('anthropometry creation should throw 422 if provided patient does not exist', async () => {
  const anthropometryAPI = new AnthropometryAPI(
    apiOptions(schema, {
      _id: new ObjectId(),
  ...
  expect(errThrown.message).toEqual(
    `Cannot create or edit this anthropometry with not existing patient ${fakePatientId}`
  );
});

test('anthropometry creation should throw 422 if provided patient does not belong to the anthropometrist', async () => {
  const anthropometryAPI = new AnthropometryAPI(
    apiOptions(schema, {
      _id: new ObjectId(),
      email: 'user-test@test.com',
  ...
  expect(errThrown.code).toEqual(422);
  expect(errThrown.message).toEqual(
    `Cannot create or edit this anthropometry with not existing patient ${patientId}`
  );
});

test('anthropometry creation should throw 422 if anthropometrist does not exist', async () => {
  const anthropometryAPI = new AnthropometryAPI(
    apiOptions(schema, {
      _id: new ObjectId(),
      email: 'user-test@test.com',
  ...
  expect(errThrown.code).toEqual(422);
  expect(errThrown.message).toEqual(
    'Cannot create or edit this anthropometry for a non existing anthropometrist'
  );
});

test('anthropometry creation should throw 403 when creating an anthropometry with a non admin user for another anthropometrist', async () => {
  const anthropometryAPI = new AnthropometryAPI(
    apiOptions(schema, {
      _id: new ObjectId(),
      email: 'user-test@test.com',
  ...
  expect(errThrown).toBeDefined();
  expect(errThrown.code).toEqual(403);
  expect(errThrown.message).toEqual(
    'Cannot create or edit this anthropometry for another anthropometrist'
  );
});

test.each(TEST_TABLE)('anthropometry should be created with admin: %#', async (testDataFunc) => {
  const anthropometryAPI = new AnthropometryAPI(
    apiOptions(schema, {
      _id: new ObjectId(),
      email: 'user-test@test.com',
  ...
  const { anthropometry } = await anthropometryAPI.create(testData);
  expect(anthropometry._id).toBeDefined();
  compareDBObject(anthropometry, testData);
});

test.each(TEST_TABLE)(
  'anthropometry should be created with common user: %#',
  async (testDataFunc) => {
    const anthropometryAPI = new AnthropometryAPI(
